...
." join ac_dept d on (o.dept_id = d.dept_id) join ac_area a on (o.area_id = a.area_id) where o.leave_id = '$leave_id'";
	...
	$day_array = array_combine(range(1,10),range(1,10));
...
."<tr><td class='Caption'>Jobs involved：</td><td class='Content'>".F::TextArea("job_involved", 5, 30, "", $row['job_involved'], " style=\"width:200px;\"")."</td>"
...
echo "<tr><td class='Caption'>&nbsp;</td><td class='Caption'>From (date)</td><td class='Caption'>To (date)</td><td class='Caption'>Time</td></tr>\n";
$leave_day_idx = 0;
...
/*
F::Submit("saveApprovedOt(".$ot_id.", 'approve')","Approve")." ";
if ($USER->CheckPages("ot","ot_final_approve")) echo F::Submit("saveApprovedOt(".$ot_id.", 'final_approve')","Final Approve")." ";
*/
if(in_array($row['status'], array('10', '20'))){
	//echo F::Submit("saveLeave(".$leave_id.", 'approve')","Approve")." ";
	if ($USER->CheckPages("leave","leave_final_approve")) echo F::Submit("saveLeave(".$leave_id.", 'final_approve')","Final Approve")." ";
