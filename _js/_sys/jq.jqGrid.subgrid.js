	...
	return this.each(function(){
		...
		$(td,t).html("<img src='"+ts.p.imgpath+"plus.gif'/>")
			.toggle( function(e) {
				...
				var atd= pos==1?'<td></td>':'';
				_id = $(res).attr("id");
				var subdata = "<tr class='jqGridSubgrid'>"+atd+"<td><img src='"+ts.p.imgpath+"line3.gif'/></td><td colspan='"+parseInt(ts.p.colNames.length-1)+"'><div id="+pID+"_"+_id+" class='jqGridTablediv'>";
				$(this).parent().after( subdata+ "</div></td></tr>" );
				$(".jqGridTablediv",ts).css("width", ts.grid.width-20+"px");
		...
		var populatesubgrid = function( rd ) {
			...
			if(!ts.grid.hDiv.loading) {
				...
				$("div.jqGridLoading",ts.grid.hDiv).fadeIn("fast");
		...
		var subGridJXml = function(sjxml, sbid){
			...
			trdiv.className="jqGridRowdiv";
			for (i = 0; i<ts.p.subGridModel[0].name.length; i++) {
				tddiv = document.createElement("div");
				tddiv.className = "jqGridCelldivth";
			...
			if (sjxml){
				...
				$("#"+pID+sbid).append($(dummy).html());
		   		...
		   		$("div.jqGridLoading",ts.grid.hDiv).fadeOut("fast");
			}
			return false;
		}
	//-------------------------
