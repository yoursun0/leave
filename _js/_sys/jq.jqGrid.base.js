...
$.fn.jqGrid = function( p ) {
	...
	$.fn.setSelection = function(selection,sd)	{
		return this.each(function(){
			...
			if (!pt.html()) {return;}
			if(!t.p.multiselect) {
				if($(pt).attr("class") !== "jqGridSubgrid") {
				if( t.p.selrow ) {$(t.rows[t.p.selindex]).removeClass("jqGridSelected");}
				t.p.selrow = $(pt).attr("id"); t.p.selindex = $(pt)[0].rowIndex;
				$(pt).addClass("jqGridSelected");
				...
				if (  ia === -1 ){ 
					if($(pt).attr("class") !== "jqGridSubgrid") { $(pt).addClass("jqGridSelected");}
					stat = true;
					$("#jqg_"+t.p.selrow,t.rows).attr("checked",stat);
					t.p.selarrrow.push(t.p.selrow);
				} else {
					if($(pt).attr("class") !== "jqGridSubgrid") { $(pt).removeClass("jqGridSelected");}
					stat = false;
					$("#jqg_"+t.p.selrow,t.rows).attr("checked",stat);
	...
	$.fn.resetSelection = function(){
		return this.each(function(){
			...
			if(!t.p.multiselect) {
				if(t.p.selrow) {
					$(t.rows[t.p.selindex]).removeClass("jqGridSelected");
					t.p.selrow = null;
				}
			} else {
				$(t.p.selarrrow).each(function(i,n){
					var ind = $(t).getInd(t.rows,n);
					$(t.rows[ind]).removeClass("jqGridSelected");
			...
			this.each(function() {
				...
				if( $t.p.altRows === true && success) {
					$($t.rows).slice(1).each(function(i){
						if(i % 2 ==1) {$(this).addClass('jqGridAlt');}
						else {$(this).removeClass('jqGridAlt');}
					});
				}
			});
		}
	...
	$.fn.addRowData = function(rowid,data,pos) {
		...
		if(data) {
			this.each(function() {
				...
				$(row).addClass("jqGirdJqgrow");
				if(t.p.multiselect) {
					td = $('<td></td>');
					$(td[0],t.grid.bDiv).html("<input type='checkbox'"+" id='jqg_"+rowid+"' class='jqGridCbox'/>");
				...
				if( t.p.altRows === true ) {
					if (pos == "last") {
						if (t.rows.length % 2 == 1)  {$(row).addClass('jqGridAlt');}
					} else {
						$(t.rows).slice(1).each(function(i){
							if(i % 2 ==1) {$(this).addClass('jqGridAlt');}
							else {$(this).removeClass('jqGridAlt');}
	...
	$.fn.hideCol = function(colname) {
		return this.each(function() {
			...
			if(fndh===true) {
				...
				if($t.p.pager && $($t.p.pager).attr("class")=="jqGridScroll" ) {
					$($t.p.pager).width(gtw);
				}
	...
	return this.each( function() {
		...
		var ts = this;
		$("<div class='jqGridLoadingui' id=lui_"+this.id+"/>").insertBefore(this);
		...
		var addMulti = function(t,row){
			...
			td = document.createElement("td");
			cbid = "jqg_"+row.id;
			$(td,t).html("<input type='checkbox'"+" id='"+cbid+"' class='jqGridCbox'/>");
			formatCol($(td,t), 0);
			row.appendChild(td);
		};
		...
		var addXmlData = function addXmlData (xml,t) {
			...
			var row,gi=0,si=0,cbid,idn, getId, f=[],rd=[],cn=(ts.p.altRows === true) ? 'jqGridAlt':'';
			...
			$(ts.p.xmlReader.root+" "+ts.p.xmlReader.row,xml).each( function( j ) {
				...
				if(j%2 == 1) {row.className = cn;} $(row).addClass("jqGirdJqgrow");
				$("tbody",t).append(row);
		...
		var addJSONData = function(data,t) {
			...
			var row,cur,gi=0,si=0,drows,idn,rd=[],cn=(ts.p.altRows===true) ? 'jqGridAlt':'';
			...
			for (var i=0;i<drows.length;i++) {
				...
				if(i%2 == 1) {row.className = cn;} $(row).addClass("jqGirdJqgrow");
				$("tbody",t).append(row);
				if(afterInsRow) {ts.p.afterInsertRow(row.id,rd);}
				rd=[];
			}		
		...
		var updatepager = function() {
			if(ts.p.pager) {
				...
				if (ts.p.loadonce) {
					...
					$(".jqGridSelbox",ts.p.pager).attr("disabled",false);
				}
				if(ts.p.pginput===true) {
					$('#sp_1',ts.p.pager).html("/"+"&nbsp;"+ts.p.lastpage );
					$('input.jqGridSelbox',ts.p.pager).val(ts.p.page);
				}
				if (ts.p.viewrecords){
		...
		var beginReq = function() {
			...
			switch(ts.p.loadui) {
				case "disable":
					break;
				case "enable":
					$("div.jqGridLoading",grid.hDiv).fadeIn("fast");
					break;
				case "block":
					$("div.jqGridLoading",grid.hDiv).fadeIn("fast");
		...
		var endReq = function() {
			...
			switch(ts.p.loadui) {
				case "disable":
					break;
				case "enable":
					$("div.jqGridLoading",grid.hDiv).fadeOut("fast");
					break;
				case "block":
					$("div.jqGridLoading",grid.hDiv).fadeOut("fast");
					$("#lui_"+ts.id).hide();
		...
		var sortArrayData = function() {
			...
			if(ts.p.multiselect) {
				$("tbody tr:gt(0)", ts.grid.bDiv).removeClass("jqGridSelected");
				$("[@id^=jqg_]",ts.grid.bDiv).attr("checked",false);
				ts.p.selarrrow = [];
			}
			if( ts.p.altRows === true ) {
				$("tbody tr:gt(0)", ts.grid.bDiv).removeClass("jqGridAlt");
				$("tbody tr:odd", ts.grid.bDiv).addClass("jqGridAlt");
			}
			ts.scrollTop = 0;
			endReq();
		};
		...
		var setPager = function (){
			var inpt = "<img class='jqGridPgbuttons' src='"+ts.p.imgpath+"spacer.gif'";
			var pginp = (ts.p.pginput===true) ? "<input class='jqGridSelbox' type='text' size='3' maxlength='5' value='0'/><span id='sp_1'></span>&nbsp" : "";
			var pgl="", pgr="";
			if(ts.p.pgbuttons===true) {
			...
			if(ts.p.rowList.length >0){
				var str="<SELECT class='jqGridSelbox'>";
			...
			if(ts.p.pgbuttons===true) {
			$(".jqGridPgbuttons",ts.p.pager).mouseover(function(e){
			...
			if(ts.p.pginput===true) {
			$('input.jqGridSelbox',ts.p.pager).keypress( function(e) {
		...
		var sortData = function (index, idxcol,reload){
			...
			if(ts.p.selrow && ts.p.datatype == "local" && !ts.p.multiselect){ $('#'+ts.p.selrow,grid.bDiv).removeClass("jqGridSelected");}
			ts.p.selrow = null;
		...
		if(this.p.multiselect) {
			this.p.colNames.unshift("<input id='cb_jqg' class='jqGridCbox' type='checkbox'/>");
			...
			$('#cb_jqg',trow).click(function(){
				...
				if (this.checked) {
					...
					$(ts.rows).slice(1).each(function(i) {
						if(!$(this).hasClass("jqGridSubgrid")){
						$(this).addClass("jqGridSelected");
					...
					$(ts.rows).slice(1).each(function(i) {
						if(!$(this).hasClass("jqGridSubgrid")){
							$(this).removeClass("jqGridSelected");
						}
					});
					ts.p.selarrrow = [];
					chk=false;
		...
		$(grid.hTable).addClass("jqGridScroll")
		...
		$(grid.hDiv)
			...
			.prepend('<div class="jqGridLoading">'+ts.p.loadtext+'</div>')
			.append(grid.hTable)
			.bind("selectstart", function () { return false; });
		if(hg) {$(grid.hDiv).hide();}
		if(ts.p.pager){
			if(typeof ts.p.pager == "string") {ts.p.pager = $("#"+ts.p.pager);}
			if( $(ts.p.pager).attr("class") == "jqGridScroll") { $(ts.p.pager).css({ width: grid.width+"px", overflow: "hidden"}).show(); ts.p._height= parseInt($(ts.p.pager).height(),10); if(hg) {$(ts.p.pager).hide();}}
			setPager();
		}
		$(ts).mouseover(function(e) {
			td = (e.target || e.srcElement);
			ptr = $(td,ts.rows).parent("tr");
			if($(ptr).hasClass("jqGirdJqgrow")) {
				$(ptr).addClass("jqGridOver");
				if(!$(td).hasClass("jqGridEditable")){
					td.title = $(td).text();
				}
			}
			e.stopPropagation();
		...
		$(grid.bDiv)
			...
			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), padding: "0px", margin: "0px", overflow: ofl2,width: (grid.width)+"px"} ).css("overflow-x","hidden").css("overflow-y", "scroll")
			.append(this);
		...
		$(grid.cDiv).append("<table class='jqGridHeader' cellspacing='0' cellpadding='0' border='0'><tr><td class='jqGridHeaderLeft'><img src='"+ts.p.imgpath+"spacer.gif' border='0' /></td><th>"+ts.p.caption+"</th>"+ ((ts.p.hidegrid===true) ? "<td class='jqGridHeaderButton'><img src='"+ts.p.imgpath+"up.gif' border='0'/></td>" :"") +"<td class='jqGridHeaderRight'><img src='"+ts.p.imgpath+"spacer.gif' border='0' /></td></tr></table>").addClass("jqGridGridHeader");
		$(grid.cDiv).insertBefore(grid.hDiv);
		if( ts.p.toolbar[0] ) {
			grid.uDiv = document.createElement("div");
			if(ts.p.toolbar[1] == "top") {$(grid.uDiv).insertBefore(grid.hDiv);}
			else {$(grid.uDiv).insertAfter(grid.hDiv);}
			$(grid.uDiv,ts).width(grid.width).addClass("jqGridUserdata").attr("id","t_"+this.id);
		...
		if(ts.p.caption) {
			...
			if(ts.p.hidegrid===true) {
				$(".jqGridHeaderButton",grid.cDiv).toggle( function(){
				...
				if(hg) { $(".jqGridHeaderButton",grid.cDiv).trigger("click"); ts.p.datatype="local";}
			}
		}
		...
		.mouseup(function (e) {
			if(grid.resizing) {
				...
				if(grid.newWidth){
					...
					if(ts.p.pager && $(ts.p.pager).attr("class")=="jqGridScroll" ) {
