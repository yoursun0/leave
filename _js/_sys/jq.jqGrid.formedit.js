...
$.fn.extend({
    searchGrid : function ( p ) {
        ...
        return this.each(function(){
            ...
            if ( $("#"+IDs.themodal).html() != null ) {
                ...
                var cM = $t.p.colModel;
                var cNames = "<select id='snames' class='jqGridSearch'>";
                var nm, hc, sf;
                ...
                var sOpt = "<select id='sopt' class='jqGridSearch'>";
                ...
                sOpt += "</select>";
                // field and buttons
                var sField  = "<input id='sval' class='jqGridSearch' type='text' size='20' maxlength='100'/>";
                var bSearch = "<input id='sbut' class='jqGridButtonsearch' type='button' value='"+p.Find+"'/>";
                var bReset  = "<input id='sreset' class='jqGridButtonsearch' type='button' value='"+p.Reset+"'/>";
    			...
    			viewModal("#"+IDs.themodal,{modal:p.modal})
                if(p.drag) DnRModal("#"+IDs.themodal,"#"+IDs.modalhead+" td.jqGridModaltext");
                $("#sbut","#"+IDs.themodal).click(function(){
                    if( $("#sval","#"+IDs.themodal).val() !="" ) {
    					...
    					if(p.dirty) $(".jqGridNo-dirty-cell",$t.p.pager).addClass("jqGridDirty-cell");
    					$t.p.page= 1;
                ...
                $("#sreset","#"+IDs.themodal).click(function(){
                    if ($t.p.search) {
    					...
    					if(p.dirty) $(".jqGridNo-dirty-cell",$t.p.pager).removeClass("jqGridDirty-cell");
                        $($t).trigger("reloadGrid");
                    }
                });
            }
    ...
    editGridRow : function(rowid, p){
        ...
        return this.each(function(){
            ...
            if ( $("#"+IDs.themodal).html() != null ) {
                $(".jqGridModaltext","#"+IDs.modalhead).text(p.caption);
                ...
                var frm = $("<form name='FormPost' id='"+frmgr+"' class='jqGridFormGrid'></form>");
                var tbl =$("<table id='"+frmtb+"' class='jqGridEditTable' cellspacing='0' cellpading='0' border='0'><tbody></tbody></table>");
                $(frm).append(tbl);
                ...
                var bS  ="<input id='sData' type='button' class='jqGridEditButton' value='"+p.bSubmit+"'/>";
                var bC  ="<input id='cData' type='button'  class='jqGridEditButton' value='"+p.bCancel+"'/>";
                $(tbl).append("<tr id='Act_Buttons'><td class='jqGridNavButton'>"+bP+"&nbsp;"+bN+"</td><td class='jqGridEditButton'>"+bS+"&nbsp;"+bC+"</td></tr>");
                ...
                $("#sData", "#"+frmtb).click(function(e){
                    var postdata = $t.p.navPostData, ret=[true,"",""], extpost={};
                    $("#FormError","#"+frmtb).hide();
                    ...
                    var j =0;
                    $(".jqGridFormElement", "#"+frmtb).each(function(i){
						var suc =  true;
                    ...
                    if(ret[0] === false) {
            			...
            			if(!p.processing) {
            				p.processing = true;
                            $("div.jqGridLoading","#"+IDs.themodal).fadeIn("fast");
                            ...
                            $.ajax({
                                ...
                                complete:function(data,Status){
                                    ...
                                    $("div.jqGridLoading","#"+IDs.themodal).fadeOut("fast");
                                }
                            });
                        }                    
                    }
            ...
            function createData(rowid,obj,tb){
                ...
                $('#'+rowid+' td',obj.grid.bDiv).each( function(i) {
            		...
            		if ( nm !== 'cb' && nm !== 'subgrid' && obj.p.colModel[i].editable===true) {
            			...
            			$(elc).addClass("jqGridFormElement");
                        trdata = $("<tr "+dc+"></tr>").addClass("jqGridFormData");
                        tdl = $("<td></td>").addClass("jqGridCaptionTD");
                        tde = $("<td></td>").addClass("jqGridDataTD")
                ...
                if(cnt>0) {
                    var idrow = $("<tr class='jqGridFormData' style='display:none'><td class='jqGridCaptionTD'>"+"&nbsp;"+"</td><td class='jqGridDataTD'><input class='jqGridFormElement' id='id_g' type='text' name='id' value='"+rowid+"'/></td></tr>")
    ...
    delGridRow : function(rowids,p) {
        ...
        return this.each(function(){
            ...
            if ( $("#"+IDs.themodal).html() != null ) {
                ...
                var tbl =$("<table id='"+dtbl+"' class='jqGridDelTable'><tbody></tbody></table>");
                // error data 
                ...
                $(tbl).append("<tr><td class='jqGridDelButton'>"+bS+"&nbsp;"+bC+"</td></tr>");
                    ...
                    if(ret[0] === false) {
                		...
                		if(!p.processing) {
                            ...
                            $.ajax({
                                url:gurl,
                                type: p.mtype,
                                data: $.extend($t.p.navPostData,{oper:"del", id:postdata}),
                                complete:function(data,Status){
                                    if(Status != "success") {
                                        ret[0] = false;
                                        ret[1] = Status+" Status: "+data.statusText +" Error code: "+data.status
    ...
    navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch) {
        ...
        return this.each(function() {       
            ...
            var navTbl = $("<table cellspacing='0' cellpadding='0' border='0' class='jqGridNavtable'><tbody></tbody></table>").height(20);
            var trd = document.createElement("tr");
            $(trd).addClass("jqGridNav-row");
            var imp = $t.p.imgpath;
            var tbd;
            if (o.edit) {
            	...
            	$(tbd).append("<table cellspacing='0' cellpadding='0' border='0' class='jqGridTbutton'><tr><td><img src='"+imp+o.editicon+"'/></td><td valign='center'>"+o.edittext+"&nbsp;</td></tr></table>")
                $(tbd).css("cursor","pointer")
            	.addClass("jqGridNav-button")
            	.click(function(){
            	...
            	.hover(
            		function () {
            			$(this).addClass("jqGridNav-hover");
            		},
            		function () {
            			$(this).removeClass("jqGridNav-hover");
            		}
            	);
            	trd.appendChild(tbd);
            	tbd = null;
            ...
            if (o.add) {
            	...
            	$(tbd).append("<table cellspacing='0' cellpadding='0' border='0' class='jqGridTbutton'><tr><td><img src='"+imp+o.addicon+"'/></td><td>"+o.addtext+"&nbsp;</td></tr></table>")
            	.css("cursor","pointer")
            	.addClass("jqGridNav-button")
            	.click(function(){
            ...
            if (o.del) {
            	...
            	$(tbd).append("<table cellspacing='0' cellpadding='0' border='0' class='jqGridTbutton'><tr><td><img src='"+imp+o.delicon+"'/></td><td>"+o.deltext+"&nbsp;</td></tr></table>")
            	.css("cursor","pointer")
            	.addClass("jqGridNav-button")
            	.click(function(){
            		var dr;
            ...
            if (o.search) {
            	...
            	$(tbd).append("<table cellspacing='0' cellpadding='0' border='0' class='jqGridTbutton'><tr><td class='jqGridNo-dirty-cell'><img src='"+imp+o.searchicon+"'/></td><td>"+o.searchtext+"&nbsp;</td></tr></table>")
            	.css({cursor:"pointer"})
            	.addClass("jqGridNav-button")
            	.click(function(){
            ...
            if (o.refresh) {
            	...
            	$(tbd).append("<table cellspacing='0' cellpadding='0' border='0' class='jqGridTbutton'><tr><td><img src='"+imp+o.refreshicon+"'/></td><td>"+o.refreshtext+"&nbsp;</td></tr></table>")
            	.css("cursor","pointer")
            	.addClass("jqGridNav-button")
            	.click(function(){
            		$t.p.search = false;
            		$t.p.page=1;
                    $($t).trigger("reloadGrid");
            		if (dirtycell) $(".jqGridNo-dirty-cell",$t.p.pager).removeClass("jqGridDirty-cell");
            		if(o.search) {
            	        var gID = $("table:first",$t.grid.bDiv).attr("id");
            ...
            if(o.position=="left") {
                $(navTbl).append(trd).addClass("jqGridNav-table-left");
            } else {
                $(navTbl).append(trd).addClass("jqGridNav-table-right");
            }
            $(elem).prepend(navTbl);
        });
    },
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			if( !this.grid) return;
			var findnav = $(".jqGridNavtable",elem)[0];
			if (findnav) {
				var tdb, tbd1;
				...
				$(tbd).append("<table cellspacing='0' cellpadding='0' border='0' class='jqGridTbutton'><tr><td>"+im+"</td><td>"+p.caption+"&nbsp;</td></tr></table>")
				.css("cursor","pointer")
				.addClass("jqGridNav-button")			
				.click(function(e){
...
function createModal(aIDs, content, p, insertSelector, posSelector, appendsel) {
    ...
    jQuery(mw).addClass("jqGridModalwin").attr("id",aIDs.themodal);
    var mh = jQuery('<div id="'+aIDs.modalhead+'"><table width="100%"><tbody><tr><td class="jqGridModaltext">'+p.caption+'</td> <td align="right"><a href="javascript:void(0);" class="jqmClose">'+(clicon!=''?'<img src="' + clicon + '" border="0"/>':'X') + '</a></td></tr></tbody></table> </div>').addClass("jqGridModalhead");
    var mc = document.createElement('div');
    jQuery(mc).addClass("jqGridModalcontent").attr("id",aIDs.modalcontent);
    jQuery(mc).append(content);
    mw.appendChild(mc);
    var loading = document.createElement("div");
    jQuery(loading).addClass("jqGridLoading").html(p.processData||"");
    jQuery(mw).prepend(loading);
    jQuery(mw).prepend(mh);
    jQuery(mw).addClass("jqmWindow");
	if (p.drag) {
		mf = document.createElement("div");
		mf = jQuery("<img  class='jqGridJqResize' src='"+p.imgpath+"resize.gif'/>");
		jQuery(mw).append(mf);
	}
...
function DnRModal(modwin,handler){
    ...
    jQuery(modwin).jqDrag(handler).jqResize(".jqGridJqResize");
	return false;
};
//Helper functions
