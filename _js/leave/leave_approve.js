...
function getApproveListByUserId(user_id){
	SubmitOptions("getApproveListByUserId", "#leaveList", {
		user_id: user_id
	}, { ok: function(html){
			$("#leaveList").html(html);
...
function saveLeave(id, action){
	...
	if(action == "approve" || action == "final_approve"){
		...
		if(leave_type == "20") {
			alert("Reminder : please check the doctor certificate before approval.");			
		}
...
function updateLeaveItem(id, json){
	if (json.error_msg) {
		...
		$(".leaveItem[rel=" + id + "]").click(function(){
			var temp_status = $(this).children().eq(1).attr("class").substring(6);
			if (temp_status == "10" || temp_status == "20") {
				$(".selectedLeave").removeClass("selectedLeave");
				dlgLeave($(this).addClass("selectedLeave").attr("rel"));
			}
		});
